...
def save_everything(directory, train_size, seed, different_multiplier, different_place, y_pred, rmses, mses, bp):
    ...
    if not os.path.exists(f"{directory}/dumps/{train_size}/{seed}/{different_multiplier}{chr(65+different_place)}"):
        os.makedirs(f"{directory}/dumps/{train_size}/{seed}/{different_multiplier}{chr(65+different_place)}")
        ...
        for different_place in different_places: # kõik erinevad kohad
            ...
            for different_multiplier in different_multipliers: # kõik erinevad korrutajad
                ...
                for seed in seeds:  # kõik seedid
                    ...
                    utils.reset_seeds(seed)
                    X_train = create_x_train(train_size, different_place=different_place, different_multiplier=different_multiplier, x_range=(0,10), n_places=5, seed=seed)
                    ...
                    rmses, rmse_fig = utils.joonista_rmses5x(model=model, start=0, end=10, steps=1000, akna_laius=0.1,
                                                            fn=lambda x: 0*x, analyytiline_myra=lambda x: 0.09*x**2+0.09, show_plt=False, title_text=descriptor)
                    rmse_fig.savefig(
