...
def train_model(
    ...
    """Mudeli treenimine, voib kasutada erinevaid funktsioone ja myra, tagastab mudeli, ajaloo ja treeningandmed

    :param trainX: treeningX andmed
    :param nn_size: vorgu kihtide suurused, hetkel ainult 2 kihti lubatud, defaults to (20, 20)
    :param optimizer: optimiseerija, hetkel valitud hea vaiksematele andmehulkadele, defaults to keras.optimizers.Adam( learning_rate=0.003, amsgrad=True, epsilon=1e-3)
    ...
    :param noise_fn: myrafn, defaults to lambdax:0.3*np.random.randn(len(x))+0.3*x*np.random.randn(len(x))
    :param reverse_fn: kas poorata fn tagurpidi, defaults to False
    :param reverse_noise: kas poorata myra tagurpidi, defaults to False
...
def main():
    ...
    for train_size in train_sizes:    # koik suurused
        ...
        for different_place in different_places:  # koik erinevad kohad
            ...
            rmses, rmse_fig = utils.joonista_rmses5x(model=model, start=0, end=10, steps=1000, akna_laius=0.1,
                                                     fn=fn, noise_fn=noise_fn, analyytiline_myra=lambda x: 0.09*x**2+0.09, reverse_noise=True, show_plt=False, title_text=descriptor)
    ...
    print("OK!", {different_multiplier}, {seed})
